...
namespace wke
{
    ...
    bool CWebView::repaintIfNeeded()
    {
        ...
        if (m_graphicsContext == NULL)
        {
			...
			if (m_handler.InitGraphicsCallback){
				m_handler.InitGraphicsCallback(this, m_handler.InitGraphicsCallbackParam, m_pixels, m_width, m_height);
			}
        }
...
#define SPI_GETWHEELSCROLLCHARS (0x006C)
	...
	void CWebView::onInitGraphics(wkeInitGraphicsCallback callback, void* callbackParam){
		m_handler.InitGraphicsCallback = callback;
		m_handler.InitGraphicsCallbackParam = callbackParam;
	}

	void CWebView::onLoader(wkeLoaderCallback callback, void* callbackParam){
		m_handler.LoaderCallback = callback;
		m_handler.LoaderCallbackParam = callbackParam;
	}
